...
def game_state(actual, required):
    states = {State.STARTING: "/create", State.PLAYING: "/start", State.VOTING: "/end"}
    return (
        "Game is in state "
        + State.STARTING.name
        + " which occurs after "
        + states[actual]
        + ". Command should be executed in state "
        + required.name
        + " which occurs after "
        + states[required]
    )

connection = sqlite3.connect("secret_thrower") #Change to supabase
