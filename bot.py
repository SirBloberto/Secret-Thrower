...
async def create(interaction: discord.Interaction, team1: discord.VoiceChannel, team2: discord.VoiceChannel, info: str=None):
    ...
    if str(guild.id) not in config:
        config[guild.id] = Config(60, []).__dict__
...
@discord.app_commands.describe(count="Number of secret throwers on each team")
async def send(interaction: discord.Interaction, count: int = 1):
    guild = interaction.guild
    ...
    game.throwers.append(random.choices(game.team1.players, k=len(game.team1.players) if len(game.team1.players) < count else count))
    game.throwers.append(random.choices(game.team2.players, k=len(game.team2.players) if len(game.team2.players) < count else count))
    for player in game.throwers:
        player = player.member
        await player.send("You are the secret thrower! Your goal is to lose the game without being discovered by others", delete_after=60.0)
    game.state = State.PLAYING
    await interaction.response.send_message(f"Secret Thrower count: {game.throwers[0]},{game.throwers[1]}", ephemeral=True, delete_after=60.0)
...
async def end(interaction: discord.Interaction, winner: discord.VoiceChannel):
    ...
    team1_string, team2_string = list_players(game)
    embed.set_field_at(0, name=game.team1.team.name, value=team1_string)
    embed.set_field_at(1, name=game.team2.team.name, value=team2_string)
...
@tree.command(name="show", description="Show the currrent config setting including current voting timer and blacklisted members")
...
async def on_reaction_add(reaction: discord.Reaction, user: discord.user):
    ...
    for i, team in enumerate(teams):
        ...
        for j, player in enumerate(team.players):
            current_reaction = [emoji for emoji in reactions if emoji.emoji == REACTIONS[i][j]]
