...
@discord.app_commands.describe(
    thrower_info="Show the number of throwers on each team and tell the throwers who their counterpart(s) are (if they exist)"
)
async def settings(
    interaction: discord.Interaction, voting_timer: typing.Optional[int], thrower_info: typing.Optional[bool]
):
    global json
    ...
    if voting_timer:
        ...
        await interaction.channel.send(
            f"Changed the Secret-Thrower voting_timer setting to {voting_timer} seconds", delete_after=60.0
        )
    if thrower_info != None:
        config[guild]["thrower_info"] = thrower_info
        await interaction.channel.send(
            f"Changed the Secret-Thrower thrower_info setting to {thrower_info}", delete_after=60.0
        )
