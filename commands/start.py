...
async def start(interaction: discord.Interaction, team1_count: int = 1, team2_count: int = 1):
    ...
    if game == None:
        return await interaction.response.send_message(
            "Not in a game! Use /create to start a Secret-Thrower game", ephemeral=True, delete_after=60.0
        )
    if game.state != State.STARTING:
        return await interaction.response.send_message(
            game_state(game.state, State.STARTING), ephemeral=True, delete_after=60.0
        )
    ...
    player_recent = cursor.execute(
        f"SELECT * FROM v_recent WHERE user_id IN ({','.join([str(player.member.id) for player in (game.team1.players + game.team2.players)])})"
    ).fetchall()
    ...
    for index in range(2):
        ...
        for player in players[index]:
            probability[index][player.member.id] = (
                recent[player.member.id] if player.member.id in recent else 0
            ) + BASE_RECENT
        ...
        for player in game.throwers[index]:
            ...
            if thrower_info:
                ...
                if len(team_throwers) == 0:
                    ...
                    message += "Your partners are: " + "".join(
                        str(thrower.member.name) + ", " for thrower in team_throwers
                    )
    ...
    cursor.execute(
        f"INSERT INTO guild (guild_id) SELECT {game.guild.id} WHERE NOT EXISTS (SELECT * FROM guild WHERE guild_id = {game.guild.id})"
    )
    cursor.execute(
        f"INSERT INTO game (game_id, game_time, guild_id, info) VALUES ({game.message.id}, ?, {game.guild.id}, ?)",
        (
            game_time,
            game.info,
        ),
    ...
    for index, team in enumerate(teams):
        cursor.execute(
            f"INSERT INTO channel (channel_id) SELECT {team.team.id} WHERE NOT EXISTS (SELECT * FROM channel WHERE channel_id = {team.team.id})"
        )
        cursor.execute(f"INSERT INTO team (game_id, channel_id, winner) VALUES ({game.message.id}, {team.team.id}, 0)")

        for player in team.players:
            cursor.execute(
                f"INSERT INTO user (user_id, username) SELECT {player.member.id}, ? WHERE NOT EXISTS (SELECT * FROM user WHERE user_id = {player.member.id})",
                (player.member.name,),
            )
            cursor.execute(
                f"INSERT INTO player (game_id, channel_id, user_id, thrower) VALUES ({game.message.id}, {team.team.id}, {player.member.id}, {int(any(thrower.member.id == player.member.id for thrower in game.throwers[index]))})"
            )
    connection.commit()
